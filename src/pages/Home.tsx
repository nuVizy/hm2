...
const Home = () => {
  ...
  return (
    <div>
      ...
      <section className="relative min-h-[90vh]">
        ...
        <div className="container relative z-10 flex min-h-[90vh] flex-col justify-center py-20">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-6 font-display text-4xl font-semibold text-sand md:text-6xl">
              {site.hero.title}
            </h1>
            <p className="mt-6 text-lg text-sand/80">{site.hero.subtitle}</p>
            <div className="mt-10 flex flex-wrap gap-4">
              <button
                type="button"
                onClick={() => openModal()}
                className="rounded-full bg-gold px-6 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-ink"
              >
                {site.hero.primaryCta}
              </button>
              <Link
                to="/services"
                className="flex items-center gap-2 rounded-full border border-sand/40 px-6 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-sand"
              >
        ...
        <div className="container">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {site.services.slice(0, 5).map((service) => (
              <div key={service.id} className="rounded-3xl border border-white/10 bg-ink/60 p-6">
                ...
                <h3 className="mt-5 text-lg font-semibold text-sand">{service.name}</h3>
                <p className="mt-2 text-sm text-sand/70">{service.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-y border-white/10 bg-ink/70 py-16">
        <div className="container">
          <div className="grid gap-10 md:grid-cols-[1.2fr_1fr]">
            ...
            <div className="grid grid-cols-2 gap-6">
              {site.badges.map((badge) => (
                <div key={badge.label} className="rounded-2xl border border-white/10 bg-ink/40 p-4">
                  <div className="aspect-square w-16 overflow-hidden rounded-xl bg-white/5">
                    <img
                      src={badge.image}
                      alt={badge.label}
                      loading="lazy"
                  ...
                  <p className="mt-3 text-xs text-sand/70">{badge.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              {site.team.slice(0, 4).map((member) => (
                <div key={member.name} className="rounded-2xl border border-white/10 bg-ink/60 p-4">
                  ...
                  <h3 className="mt-4 text-sm font-semibold text-sand">{member.name}</h3>
          ...
          <div className="rounded-3xl border border-white/10 bg-ink/70 p-8">
            ...
            <div className="mt-6 grid gap-6">
              {site.testimonials.slice(0, 3).map((testimonial) => (
                <div key={testimonial.name} className="rounded-2xl border border-white/10 bg-ink/50 p-5">
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-sand/60">{testimonial.role}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <section className="border-y border-white/10 bg-ink/70 py-16">
        <div className="container grid gap-8 lg:grid-cols-[1.2fr_1fr]">
          <div>
            ...
            <p className="mt-4 text-sm text-sand/70">{site.partners[0].story}</p>
      ...
      <section className="py-20">
        <div className="container">
          <div className="rounded-3xl bg-gold px-8 py-12 text-ink md:px-16">
            <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
              ...
              <button
                type="button"
                onClick={() => openModal()}
                className="rounded-full bg-ink px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-sand"
              >
                Start an enquiry
              </button>
            </div>
