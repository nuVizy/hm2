        ...
        <div className="container grid gap-8 lg:grid-cols-2">
          {site.services.map((service) => (
            <div key={service.id} className="rounded-3xl border border-white/10 bg-ink/60 p-6">
              ...
              <div className="mt-6">
                <h2 className="text-2xl font-semibold text-sand">{service.name}</h2>
                <p className="mt-2 text-sm text-sand/70">{service.description}</p>
                <ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-sand/70">
                ...
                <button
                  ...
                  className="mt-6 rounded-full border border-gold px-6 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-gold"
                >
                  Enquire now
                </button>
              </div>
        ...
        <div className="container">
          ...
          <div className="mt-8 grid gap-4">
            {[
              ...
              <details
                key={item.title}
                className="rounded-2xl border border-white/10 bg-ink/50 px-6 py-4"
              >
                <summary className="cursor-pointer text-sm font-semibold text-sand">
                  {item.title}
                </summary>
                <p className="mt-3 text-sm text-sand/70">{item.body}</p>
              </details>
            ))}
          </div>
        </div>
