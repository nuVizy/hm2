...
export const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div className={`rounded-3xl border border-line bg-white shadow-card ${className}`}>
    {children}
  </div>
);
...
export const Button: React.FC<{
  ...
  const base =
    "inline-flex items-center justify-center rounded-full px-6 py-3 text-xs font-semibold uppercase tracking-[0.25em] transition";
  const variants = {
...
export const Badge: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <span
    className={`inline-flex items-center rounded-full border border-line bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-muted ${className}`}
  >
    {children}
  </span>
);
